...
class PendingContainer extends Component {
   ...
   deleteMeeting = (id) => {
      ...
      axios.delete('https://blind-meetup.dm.hs-furtwangen.de/Blind_MeetUp/public/api/meeting/' + id).then((response) => {
         this.props.loadTask();
      });
   }
            ...
            <div className="meetUps">
               ...
               <Slider {...settings}>
                  {this.props.meetings.meeting.map((e, i) => {
                     ...
                     return (
                        <JointMeeting key={i} place={e.place} date={this.formateDate(e.date)} time={time} day={t(this.getThisDay(e.date))} />
                     )
                  })}
               </Slider>
            </div>
