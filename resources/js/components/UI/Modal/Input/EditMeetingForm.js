...
const EditMeetingForm = props => {
    ...
    const updateMeeting = () => {
        axios.put('/Blind_MeetUp/api/meeting/' + props.editMeetingData.id, { type: type, place: place, date: date, members: members, max_members: max_members, rating: rating, img_link: img_link })
            .then((response) => {
                props.loadTask();
                props.modalHandler();
                resetEditState();
